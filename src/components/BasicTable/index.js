        ...
        <Paper sx={{ width: '100%', overflow: 'hidden' }}>
            <TableContainer sx={{ maxHeight: "63vh" }}>
                ...
                <TableBody {...getTableBodyProps()}>
                                ...
                                <TableRow {...row.getRowProps()}>
                                    {row.cells.map((cell) => {
                                        ...
                                        if(cell.column.Header === "Faculty Name" || cell.column.Header === "Name") {
                                            return <TableCell>{cell.row.original.firstName + " " + cell.row.original.middleName?.toUpperCase() + " " + cell.row.original.lastName}</TableCell>
                                        }
                                        ...
                                        if(cell.column.Header === "Date") {
                                            return <TableCell>{cell.row.original.date}</TableCell>
                                        }

                                        if(cell.column.Header === "Role Name") {
                                            return <TableCell><b>{cell.row.original.name.toUpperCase()}</b></TableCell>
                                        }

                                        if(cell.column.Header === "Time") {
                                            return <TableCell>{convertTo112HourFormat(cell.row.original.time)}</TableCell>
                                        }
                                        ...
                                        if(cell.column.Header === "Health Status") {
                                            return <TableCell style={{ fontWeight: "bold", color: cell.row.original.userHealthStatus === "positive" ? "red" : "#30b8a6" }}>
                                                {cell.row.original.userHealthStatus}
                                            </TableCell>
                                        }

                                        if(cell.column.Header === "Registration Date") {
                                            return <TableCell>{cell.row.original.createdAt.split('T')[0]}</TableCell>
                                        }

                                        if(cell.column.Header === "Action") {
                                            return <TableCell className={cell.row.original.action === "Scanned the QR Code" ? 'entry' : 'exit' } >{cell.row.original.action}</TableCell>
                                        }
                                        //Permissions
                                        if(cell.column.Header === "Permissions") {
                                            return <TableCell><div className='permission-container'>
                                                {
                                        ...
                                        if(hasTracing && cell.column.Header==='Actions') {
                                            return <TableCell className='iconBtnWrapper'>
                    ...
                    {
                        tableData.length === 0 && <TableRow><TableCell>No data to be displayed at the moment</TableCell></TableRow>
                    }
                </TableBody></>
                }
            </Table>
